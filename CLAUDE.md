...
**Bug, root cause corrected 2026-07-15 from a real downloaded `IntroscopeAgent.log` — `UnknownAgent` in the PHP probe's metric path:** per Broadcom's [PHP agent naming docs](https://techdocs.broadcom.com/us/en/ca-enterprise-software/it-operations-management/dx-apm-agents/SaaS/php-agent/monitor-php-applications-with-ca-digital-experience-insights/php-agent-naming-ca-digital-experience-insights.html), the PHP probe's metric path takes the form `host|php-probes|<AgentName>(<program>)`, where `<AgentName>` is the `{collector}` variable — "Name of Infrastructure Agent," resolved from the IA's own `introscope.agent.agentName` at the moment the PHP probe first registers. Confirmed live via `nass query` (2026-07-10): the Kubernetes deployment's PHP probe reports under `SuperDomain|bpa-demo-k8s|php-probes|UnknownAgent(/usr/sbin/apache2)` — a generic fallback, not `bpa-demo-k8s` — while Docker Compose's own PHP probe correctly resolved to `bpa-demo-docker(/usr/sbin/apache2)` once its identity was fixed (see "Deployment identity" above). **First fix attempt (shipped, then disproved):** a wait loop polling `APMIA_PHP_COLLECTOR_HOST:APMIA_PHP_COLLECTOR_PORT` via `/dev/tcp` before starting Apache, on the theory that the PHP probe's first registration could race ahead of the IA's own identity resolution. Deployed and retested live — `UnknownAgent` still occurred. **Real root cause, found 2026-07-15 by reading a real `IntroscopeAgent.log` pulled directly off a live pod's `dx-o2-agent` container filesystem:** the PHP-collector TCP port opening (and the PHP probe's first ARF connection through it) has nothing to do with when the IA resolves its own agent name for EM reporting purposes — the log showed the collector port opening and the probe's first ARF connection at ~23-24s after IA startup, while the IA's actual WSS handshake to the SaaS Enterprise Manager didn't succeed until ~104s after startup, and only on a *second* attempt (the first failed with a `NullPointerException` in the WebSocket handshake — `ChannelFuture.await(long)` on a null `handshakeFuture()` — followed by `serverWentAway`). The probe's `{collector}` identity is resolved (or, when this races, falls back to `UnknownAgent`) at the moment of that first ARF connection — a full 80+ seconds *before* the EM session that identity actually depends on even exists. The TCP-port wait was therefore checking a condition that was already true long before the condition that mattered. **Corrected fix:** replaced the port-check wait loop with a wait on the IA's own log file (`${APMIA_HOME}/logs/IntroscopeAgent.log`, already readable from `apache-php` via the shared `apmia-share` volume — no network call) for the literal success line `Connected controllable Agent to the Introscope Enterprise Manager`, capped at 90s (comfortably above the 104s-after-a-failed-first-attempt case actually observed), with the same warn-and-continue-anyway fallback and skip-when-probe-inactive guard as before. Verified in isolation (grep condition confirmed to match the real downloaded log) and via `bash -n`/ASCII-cleanliness checks on `entrypoint.sh` — **not yet confirmed to actually fix `UnknownAgent` live**, since that still requires rebuilding the image and redeploying to the Kubernetes cluster (no cluster access from this sandbox).
