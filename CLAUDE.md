...
3. Resolves `?page=<slug>` against a static `$routes` array. Apache mod_rewrite maps clean URLs (`/shop`, `/basket`, `/product`, etc.) directly to `index.php?page=<slug>` via `vhost.conf`; bare `/` and `/index.php` default internally to `page=shop`.
