...
**Bug (hypothesis-driven fix, not yet observed live) — `UnknownAgent` in the PHP probe's metric path:** per Broadcom's [PHP agent naming docs](https://techdocs.broadcom.com/us/en/ca-enterprise-software/it-operations-management/dx-apm-agents/SaaS/php-agent/monitor-php-applications-with-ca-digital-experience-insights/php-agent-naming-ca-digital-experience-insights.html), the PHP probe's metric path takes the form `host|php-probes|<AgentName>(<program>)`, where `<AgentName>` is the `{collector}` variable — "Name of Infrastructure Agent," resolved from the IA's own `introscope.agent.agentName` at the moment the PHP probe first registers. Confirmed live via `nass query` (2026-07-10): the Kubernetes deployment's PHP probe reports under `SuperDomain|bpa-demo-k8s|php-probes|UnknownAgent(/usr/sbin/apache2)` — a generic fallback, not `bpa-demo-k8s` — while Docker Compose's own PHP probe correctly resolved to `bpa-demo-docker(/usr/sbin/apache2)` once its identity was fixed (see "Deployment identity" above). Root cause: `entrypoint.sh` starts Apache immediately with no wait for the `dx-o2-agent` sidecar's PHP-collector socket to actually be listening — in Kubernetes, `apache-php` and `dx-o2-agent` are separate containers in the same pod with no guaranteed start order, and the sidecar's JVM cold-start + WSS handshake to the SaaS EM can take longer than Apache's own startup (especially under `dxo2.resources.limits.cpu: 500m`), so the PHP probe's first registration attempt can race ahead of the Infrastructure Agent resolving its own identity and fall back to the generic placeholder — which then persists for the life of that Apache process. Docker Compose's lighter, unconstrained startup profile makes it far less likely to lose this race, which is why it resolved correctly there without any fix. Fixed by adding a wait loop before `service cron start`/`apache2ctl configtest`/`exec apache2`: polls `APMIA_PHP_COLLECTOR_HOST:APMIA_PHP_COLLECTOR_PORT` via a bash `/dev/tcp` connect-check every second, up to 60s, and starts anyway with a warning if it never comes up — skipped entirely when the probe volume isn't mounted (DX O2 not deployed), so a vanilla deployment's startup time is unaffected. Verified in isolation (both the immediate-success and 60s-timeout-then-warning paths return exactly as expected) and inside the real `apache-php` image (confirmed it correctly blocks Apache from starting while the collector is unreachable) — **not yet confirmed to actually fix `UnknownAgent`**, since that requires rebuilding the image, redeploying to the live Kubernetes cluster (no cluster access from this sandbox), and observing whether new PHP-probe metrics resolve to `bpa-demo-k8s` going forward.
