...
**Status: worked around again as of 2026-09-02, opt-in via `APP_TYPE=mp` (the
current default).** Reverted 2026-08-19, then reinstated behind the `APP_TYPE`
`.config` variable at the user's request — see the `TODO` file and CLAUDE.md's
"Front controller" section. BPA-Demo uses per-page wrapper files at the
document root — each wrapper (`shop.php`, `basket.php`, etc.) runs one
non-include statement before `require __DIR__ . '/index.php'`:
...
with `vhost.conf-mp` mapping `/shop` → `shop.php?page=shop` (not
`index.php?page=shop`) so `SCRIPT_NAME=/shop.php` and both gates pass. Every
internal link/redirect is generated by `app/src/lib/routing.php`'s
`page_url()`, which knows the active mode from the same `APP_TYPE` variable —
see that file and CLAUDE.md for the mechanism. Setting `APP_TYPE=plain`
returns to the pre-2026-09-02 behavior: no wrapper files are reached, browser-
agent auto-injection does not occur anywhere in the app, and this remains a
deliberate demonstration of the DX O2 shortcoming, not a bug to chase.
See `bug_php_probe.md` for the full original diagnosis.
