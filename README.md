...
| Stylesheet delivery | All CSS is served as a separate static file (`/css/app.css`); no inline `<style>` block in HTML responses — keeps HTML responses small, enables browser caching, and ensures `</head>` appears at byte ~239 for reliable browser-agent injection |
