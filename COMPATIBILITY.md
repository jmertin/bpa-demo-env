...
> **Browser-agent Frontend start + SCRIPT_NAME requirement — not currently worked around.** The PHP probe has two gates for BA injection. (1) The very first PHP opcode in the entry script must be a non-include opcode; otherwise the probe enters include-tracking mode and skips BA entirely. (2) `SCRIPT_NAME` (not `REQUEST_URI`) must not be `index.php` or bare `/` — the probe treats those as null segments. BPA-Demo's plain front-controller pattern (all requests through `index.php`) fails both gates, so browser-agent auto-injection does not currently occur. A per-page-wrapper-file workaround previously satisfied both gates but was reverted at the user's request on 2026-08-19 — see `bug_php_probe.md` for the full diagnosis and vendor-facing fix request.
