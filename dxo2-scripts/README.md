...
| `bpa-demo-services-universe.sh` | The `"BPA Demo universe"` **O2/Platform Universe** (`dx-do o2-universe` -- a.k.a. "Services Universe" in the console; a different resource from the APM Universe above, with its own id space) -- created because the tenant needs both universe types until the product merges them. `o2-universe create` accepts no scoping parameters at all (extras are silently ignored), so this Universe is created unscoped (`{"filter": {"op": "ALL"}}` on both its `tas` and `nass` views -- matches the whole tenant, not just BPA-Demo). There is no `update`/`add-view` command to narrow it afterward, and no `delete` command either (this script uses `apm-universe delete`, which works across both universe types). Narrowing the scope to BPA-Demo requires a manual console step -- see the script's header comment and `BUGS`. |
